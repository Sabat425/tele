(function () {
   ...
   const tel_download_video = (url) => {
    ...
    // Try to extract filename from stream metadata
    try {
    ...
    const save = () => {
      ...
      // Trigger download using click() for both mobile and desktop
    ...
    const createDownloadButton = (videoElement) => {
      ...
      // Function to add hover effect to video container
      const addHoverEffect = (videoContainer, buttonContainer) => {
      ...
      const addDownloadButtonsToVideos = () => {
        // Find all video elements that don't already have download buttons
        const videos = document.querySelectorAll('video:not(.has-download-button)');
        ...
        videos.forEach(video => {
          ...
          const buttonContainer = createDownloadButton(video);
          videoContainer.appendChild(buttonContainer);
          
          // Add hover effect
          addHoverEffect(videoContainer, buttonContainer);
        });
      };
    
      // Run periodically to catch dynamically added videos
      setInterval(addDownloadButtonsToVideos, REFRESH_DELAY);
